use lectern::resolver::packagist::{P2Version, P2Dist, P2Source, search_packagist, fetch_packagist_versions_cached};
...
mod packagist_tests {
    ...
    fn test_p2_version_structure() {
        ...
        other.insert("description".to_string(), serde_json::Value::String("Test package".to_string()));
        other.insert("license".to_string(), serde_json::Value::Array(vec![serde_json::Value::String("MIT".to_string())]));
        ...
        assert_eq!(dist.url, Some("https://example.com/package.zip".to_string()));
        
        let source = p2_version.source.as_ref().unwrap();
        assert_eq!(source.stype, Some("git".to_string()));
        assert_eq!(source.url, Some("https://github.com/test/repo.git".to_string()));
        
        assert_eq!(p2_version.other.get("description").unwrap().as_str().unwrap(), "Test package");
    ...
    async fn test_search_packages_integration() {
        ...
        let result = search_packagist(&vec!["guzzle".to_string()]).await;
    ...
    async fn test_get_package_versions_integration() {
        ...
        match result {
            Ok(versions) => {
                ...
                let has_valid_version = versions.iter().any(|v| {
                    v.version.chars().next().unwrap_or('x').is_ascii_digit()
                });
                assert!(has_valid_version, "Expected to find versions starting with digits");
                
                // Check that some version has required metadata
                let has_metadata = versions.iter().any(|v| v.dist.is_some() || v.source.is_some());
                assert!(has_metadata, "Expected versions to have dist or source metadata");
            }
            Err(_) => {
                // Network might not be available, skip this test
                println!("Skipping network-dependent test");
    ...
    fn test_p2_version_metadata_extraction() {
        let mut other = serde_json::Map::new();
        other.insert("description".to_string(), serde_json::Value::String("A test description".to_string()));
        other.insert("keywords".to_string(), serde_json::Value::Array(vec![
            serde_json::Value::String("test".to_string()),
            serde_json::Value::String("package".to_string()),
        ]));
        other.insert("homepage".to_string(), serde_json::Value::String("https://example.com".to_string()));
        other.insert("license".to_string(), serde_json::Value::Array(vec![
