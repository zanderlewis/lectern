...
mod integration_tests {
    ...
    "require": {
        ...
        assert!(build_output.status.success(), "Failed to build lectern: {}", 
    ...
    fn test_lectern_search_command() {
        ...
        assert!(stdout.contains("monolog"), "Expected search results to contain 'monolog'");
    }
    ...
    fn test_lectern_invalid_command() {
        ...
        assert!(!stderr.is_empty(), "Expected error message for invalid command");
    }
    ...
    "require": {
        ...
        if !install_output.status.success() {
            eprintln!("Install failed: {}", String::from_utf8_lossy(&install_output.stderr));
        }
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("monolog"), "Status should show installed monolog package");
    }
    ...
    fn get_lectern_binary_path() -> String {
        ...
        root.join("target").join("release").join("lectern").to_string_lossy().into_owned()
    }
}
